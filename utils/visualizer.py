...
def compute_psnr(img1, img2):
    ...
    return np.mean(msr)**0.5, out_mean, max1
...
def MetricsCal(GT, P, scale):# c,w,h
    ...
    m0, m1, GTmax = compute_psnr(GT, P) # bandwise mean psnr
    ...
    return np.float64(m0), np.float64(m1), np.float64(m2), m3, m4, m5
...
class Visualizer():
    ...
    def plot_psnr_sam(self, visuals, image_name,
        ...
        rmse, psnr, sam, ergas, ssim, uqi = MetricsCal(real_hsi, rec_hsi, 8) # scale is not equal
        self.plot_precision['Y'][image_name[0]].append([sam, psnr])
        '''save txt'''
        write_message = "Epoch:{} Name:{} RMSE:{} PSNR:{} SAM:{} ERGAS:{} SSIM:{} UQI:{}".format(epoch+counter_ratio, image_name[0], round(rmse, 5), round(psnr,5), round(sam,5), round(ergas,5), round(ssim,5), round(uqi,5))
        with open(self.precision_path, "a") as precision_file:
        ...
        savefiles.close()
        # np.save(os.path.join(self.save_hhsi_path, "epoch_{}_real_{}.npy".format(epoch, image_name[0])), real_hsi)
        # np.save(os.path.join(self.save_hhsi_path, "epoch_{}_rec_{}.npy".format(epoch, image_name[0])), rec_hsi)
